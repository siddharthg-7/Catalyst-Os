::: {align="center"}
🚀 Catalyst OS
Autonomous Startup Operating System & AI Executive Team
```{=html}
<p align="center">
```
`<img src="https://img.shields.io/badge/Status-MVP--Development-brightgreen?style=for-the-badge" alt="Status" />`{=html}
`<img src="https://img.shields.io/badge/React.js-Frontend-blue?style=for-the-badge&logo=react" alt="React" />`{=html}
`<img src="https://img.shields.io/badge/FastAPI-Python-009688?style=for-the-badge&logo=fastapi" alt="FastAPI" />`{=html}
`<img src="https://img.shields.io/badge/Gemini-AI-4285F4?style=for-the-badge&logo=google" alt="Gemini" />`{=html}
`<img src="https://img.shields.io/badge/PostgreSQL-pgvector-336791?style=for-the-badge&logo=postgresql" alt="PostgreSQL" />`{=html}
```{=html}
</p>
```
An AI-powered operating system for founders that coordinates Finance,
Hiring, Growth, Legal, Investment, and Operations through collaborating
AI executives. One founder command triggers an entire executive
workflow---from task decomposition and cross-department consultation to
explainable recommendations, human approval, and startup-state updates.
:::
`<br />`{=html}
🌟 Key Features
👋 Smart Founder Onboarding
Two onboarding paths: Start fresh or import an existing startup.
Startup context capture: Industry, stage, team size, revenue,
burn rate, runway, funding, budget, and timeline.
Document intelligence: Upload pitch decks, financial statements,
roadmaps, hiring documents, policies, contracts, and business plans.
Shared context: Structured startup data becomes grounded context
for relevant AI executives.
📊 Executive Dashboard
Startup Health Score with Finance, Hiring, Growth, and
Operations sub-scores.
Today's Priorities with proactive alerts.
Executive Cards showing task, status, and latest recommendation.
Pending Approvals, Activity Timeline, Notifications, and
Daily Executive Brief.
Founder Command Box for text or voice objectives.
🧠 CEO Orchestrator
Understands high-level founder goals.
Decomposes goals into business tasks.
Selectively routes work to relevant departments.
Coordinates cross-agent consultation.
Resolves conflicts.
...
Budget validation.
Hiring affordability checks.
Monthly forecasting.
Scenario simulation.
👩💼 Head of Talent
Resume analysis and candidate ranking.
Skill matching and hiring scores.
Job-description generation.
Outreach email drafts.
Interview-slot suggestions.
CFO consultation before budget-sensitive hiring recommendations.
📈 Head of Growth
Go-to-market strategy.
Marketing and campaign plans.
LinkedIn and social content drafts.
...
Covers hiring, spending, campaigns, legal documents, interview
invitations, launch changes, and external communications.
Rejected actions are logged without silently changing startup state.
💡 Explainable AI
``` text
Recommendation:
Hire Rahul
...
Stores timestamp, founder command, executive, recommendation, reason,
confidence, evidence, founder decision, and final status.
...
Operations     82
📂 Knowledge Base & RAG
Upload pitch decks, financial reports, roadmaps, policies,
contracts, business plans, and resumes.
Retrieve only relevant chunks.
Ground recommendations in startup-specific evidence.
Store semantic memory using PostgreSQL and pgvector.
🎯 Scenario Simulator
Ask "What if we hire three more engineers?" and recalculate runway,
burn rate, Health Score, hiring plan, launch date, and marketing
timeline.
🎙️ Voice Interface
Issue founder commands using voice, transcribe with Whisper /
Faster-Whisper, and route through the CEO Orchestrator.
🌅 Daily Executive Brief
``` text
Good Morning.
...
Critical calculations use Python functions, SQL queries, validated
business logic, and structured source data. The LLM explains results;
deterministic tools calculate critical numbers.
🔍 Auditor Agent
``` text
Executive Outputs
       │
...
---
Finance                         Checking runway • Forecasting
expenses • Validating budget •
Reviewing burn rate
Hiring                          Creating JD • Searching applicants
• Ranking resumes • Scheduling
interviews • Drafting emails
Growth                          Planning campaign • Building GTM
strategy • Forecasting user
acquisition • Generating content
Legal                           Reviewing compliance • Preparing
contracts • Validating launch
readiness • Checking policy
requirements
Investments                     Analyzing KPIs • Reviewing
fundraising readiness • Drafting
investor update • Evaluating
valuation signals
CEO / Orchestrator              Breaking down objective •
...
```
> **Security Note:** Never commit real API keys, passwords, access
> tokens, private keys, or production credentials to Git.
...
Founder Workflow
Register or log in.
Create or import a startup.
Complete smart onboarding.
Upload startup documents.
Open the Executive Dashboard.
Enter one high-level command.
Watch relevant executives collaborate.
Review the Board Vote.
Expand Why? to inspect evidence and confidence.
Approve, reject, or edit.
View the updated dashboard and Health Score.
...
```
Business data should be scoped using `user_id`, `startup_id`, and
`workspace_id` to prevent context leakage.
`<br />`{=html}
📈 Scalability
...
Reducing hallucinations with structured outputs and evidence
validation.
...
`<br />`{=html}
💎 Product Philosophy
> **"Does this reduce the founder's workload by automating an entire
> workflow, or is it just another AI tool?"**
If it automates an end-to-end workflow such as hiring, launching,
budgeting, or planning, it belongs in Catalyst OS.
If it only generates isolated content, it is not enough.
> **That is the difference between an AI assistant and an AI operating
> system.**
`<br />`{=html}
👥 Authors
...
`<br />`{=html}
💬 Feedback
If you have feedback, suggestions, or feature requests, feel free to
open an issue in the repository or connect with the team through GitHub.
`<br />`{=html}
⭐ Support
...
::: {align="center"}
🚀 Catalyst OS
One Founder. One Command. An Entire AI Executive Team.
`<i>`{=html}Today's founders do not need another AI tool. They need an
operating system that thinks, collaborates, explains, and executes
alongside them.`</i>`{=html}
:::
